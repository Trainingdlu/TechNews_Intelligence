...
def _case_dir(tmp_path: Path) -> Path:
    path = tmp_path / "encoding_guard_case"
    path.mkdir(parents=True, exist_ok=True)
    return path


def test_scan_repository_detects_mojibake_and_replacement(tmp_path: Path) -> None:
    root = _case_dir(tmp_path)
    ...
    assert {"mojibake_marker", "replacement_char"}.issubset(kinds)


def test_scan_repository_detects_utf8_bom(tmp_path: Path) -> None:
    root = _case_dir(tmp_path)
...
def test_main_soft_mode_zero_strict_mode_nonzero(tmp_path: Path) -> None:
    root = _case_dir(tmp_path)
