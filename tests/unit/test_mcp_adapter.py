...
def test_mcp_client_with_stdio_backend(tmp_path: Path) -> None:
    tmp_dir = tmp_path / "mcp_stdio_case"
...
def test_stdio_mcp_ignores_interleaved_notifications(tmp_path: Path) -> None:
    tmp_dir = tmp_path / "mcp_notify_case"
    tmp_dir.mkdir(parents=True, exist_ok=True)
...
def test_stdio_mcp_timeout_returns_transport_error(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("AGENT_MCP_RPC_TIMEOUT_SEC", "1")
    tmp_dir = tmp_path / "mcp_timeout_case"
    tmp_dir.mkdir(parents=True, exist_ok=True)
