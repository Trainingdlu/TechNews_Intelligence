...
def build_approve_signature(record_id: int, exp: int) -> str:
...
def build_signed_approve_url(record_id: int) -> str | None:
    if not APPROVE_LINK_SECRET.strip():
        return None
    exp = int(time.time()) + APPROVE_LINK_TTL_SEC
    sig = build_approve_signature(record_id, exp)
    return f"{API_BASE_URL}/approve/{record_id}?exp={exp}&sig={sig}"


def is_valid_approve_signature(record_id: int, exp: int | None, sig: str | None) -> bool:
    ...
    if exp < int(time.time()):
        return False
    try:
        expected = build_approve_signature(record_id, exp)
    except Exception:
        return False
    return hmac.compare_digest(expected, sig)


def render_approve_confirmation_page(record_id: int, exp: int, sig: str) -> str:
