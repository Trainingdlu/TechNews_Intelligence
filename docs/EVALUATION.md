...
被评测对象是一个 **LangGraph StateGraph 智能体**：上下文准备 → 意图判断 → 工具选择 → 工具规划 → 策略检查 → 工具执行 → 证据归一 → 是否继续 → 最终综合，并包含澄清与证据不足分支；图后再执行输出 URL 守卫和来源装饰。它读取 PostgreSQL + pgvector 的科技新闻库，回答必须由工具证据支撑。
