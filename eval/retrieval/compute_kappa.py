...
def main() -> int:
    ...
    lines: list[str] = []
    lines.append("# G2 Relevance Calibration — LLM vs Human")
    lines.append("")
    lines.append(f"Labeled pairs: **{n}**" + (f" (unlabeled remaining: {unlabeled})" if unlabeled else ""))
    lines.append("")
    lines.append("| Metric | Value |")
    lines.append("|---|---|")
    lines.append(f"| Raw agreement (3-class) | {raw_agree*100:.1f}% |")
    lines.append(f"| Cohen's kappa (nominal) | {kappa:.3f} |")
    lines.append(f"| Quadratic-weighted kappa (ordinal) | {qwk:.3f} |")
    lines.append(f"| Binary-relevance agreement (rel>=1) | {bin_agree*100:.1f}% |")
    lines.append(f"| Binary-relevance Cohen's kappa | {bin_kappa:.3f} |")
    lines.append("")
    lines.append("Interpretation (Landis & Koch): <0 poor, 0-0.2 slight, 0.2-0.4 fair, 0.4-0.6 moderate, 0.6-0.8 substantial, 0.8-1.0 almost perfect.")
    lines.append("")
    lines.append("## Confusion matrix (rows = human, cols = LLM)")
    lines.append("")
    lines.append("| Human \\ LLM | 0 | 1 | 2 |")
    lines.append("|---|---|---|---|")
    for i in range(3):
