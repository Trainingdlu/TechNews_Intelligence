...
def main() -> int:
    ...
    for case in cases:
        ...
        for run_index in range(1, int(args.runs_per_case) + 1):
            ...
            predicted_intent_label = "clarification_required" if clarification_triggered else (tool_calls[0] if tool_calls else "none")
        ...
        print(
            "[TaskEval] case=%s tool_path_hit=%s retrieval_gold_hit=%s error_rate=%.3f"
            % (
                case.get("case_id"),
                _fmt_metric(layers["tool"]["acceptable_path_hit_rate"]),
    ...
    print(
        "[TaskEval] cases=%s elapsed=%.3fs intent_top1=%.3f path_hit=%s retrieval_recall10=%s "
        "analysis_support=%s system_error=%.3f output=%s"
        % (
            len(case_rows),
            elapsed,
            float(summary["intent"]["top1_accuracy"]),
            _fmt_metric(summary["tool"]["acceptable_path_hit_rate"]),
