...
RGB negative-rejection 子集只喂"含干扰、不含答案"的文档(top-5 negative),压测 synthesizer 会不会把证据外内容**冒充成"根据证据"**(误归因 = 真幻觉,唯一的失败模式)。在 final-synthesis 节点加入 Grounding & Refusal 硬规则后,deepseek 在**同一 299 题**上:
...
**配对翻转(同题逐条对比):** 修好 49 题(基线冒充证据 → 加固干净)、新引入 18 题(基线干净 → 加固冒充)、净 −31。与 generation 一致,加固硬规则让模型更保守,回答率从 57.9% 降到 44.8%,换来误归因近乎砍半。

## 三种行为的口径

agent 被喂"只含干扰、不含答案"的文档时存在三种情况:

1. **拒答**("证据不足")—— 安全。
2. **标注假设补充**("证据不足,但据我所知…(非证据)")—— 有用且诚实,不算幻觉。
3. **冒充出处**("根据证据…",而证据并没说)—— 误归因 = 真幻觉。
...
- deepseek 加固后(11.4%)**仍高于** gemini(5–8.4%):换 deepseek 省成本但牺牲拒答质量,加固是**缓解而非追平**。这是迁移的已知权衡。

## 指标口径

- 数据:RGB 中文 negative-rejection 子集,每条只喂 `negative`(不含答案)文档 top-5,经**生产接地 prompt** 合成答案。已剔除答案泄漏进所喂文档的 case(中文/阿拉伯数字归一后),剩 299/300。
- `misattributed`(LLM judge):把证据外内容冒充成"证据支撑"= 真幻觉;标注"假设/外部知识"的不算。这是核心失败指标。
- `gold_present`(确定性字符串匹配):标准答案字串原样出现在输出里。文档不含答案 → 出现 = 模型自供。硬拒答率含少量"答对但字串未逐字命中",略偏高,不是主指标。
- `answered`(LLM judge):给了实质性回答(哪怕先说"证据不足"再补充)。

## 剔除样本(1/300)· RGB 标注泄漏实例

RGB 用**阿拉伯数字答案精确匹配**给文档打 `negative` 标签,会漏掉用中文数字写出答案的文档。本子集恰有 1 例(`id=0`),剔除后固定为 299/300:

- **问题**:印控克什米尔地区寺庙踩踏事故死亡人数
- **标准答案**:`12人`
- **被标为 `negative`(本应不含答案)的 top-1 文档**:"…已造成**十二人**死亡、二十人受伤…"——答案以中文数字"十二"出现,`"12"` 字串不出现,故逃过 RGB 的 negative 标注。

该文档其实含答案:若留下,模型据此作答属**合法接地**,却会被误归因指标错判成幻觉,污染分子。`_answer_leaks_into_docs`(`run_rgb_rejection.py`)对答案做数字归一(`12 → 十二 / 一二`)后比对所喂 top-5,识破泄漏并剔除。各 run 同样剔此 1 条 → 天然同题 299,加固 before→after 可逐条配对。
