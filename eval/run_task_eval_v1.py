"""Run task-driven eval dataset (v1) with layered scoring."""
...
try:
    ...
    from task_eval_v1_schema import load_cases_jsonl
    from task_eval_v1_scoring import aggregate_layer_summary, score_case
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run task-driven eval v1 (layered).")
    parser.add_argument(
        "--dataset",
        type=Path,
        default=eval_dir / "datasets" / "task_eval_v1_cases.jsonl",
        help="Case dataset JSONL generated by build_task_dataset_v1.py.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=eval_dir / "reports" / "task_eval_v1_latest.json",
...
def main() -> int:
    ...
    for case in cases:
        ...
        for run_index in range(1, int(args.runs_per_case) + 1):
            request_id = f"task-eval-v1-{case['case_id']}-{run_index}-{uuid.uuid4().hex[:8]}"
        ...
        print(
            "[TaskEvalV1] case=%s tool_path_hit=%.3f retrieval_gold_hit=%s error_rate=%.3f"
            % (
                case.get("case_id"),
    ...
    print(
        "[TaskEvalV1] cases=%s elapsed=%.3fs intent_top1=%.3f path_hit=%.3f retrieval_recall10=%s "
